...
import { Card, Col, Collection, Modal, Row, Button, Icon, Dropdown } from "react-materialize";
          ...
          <Row>
            <Col s={12} id="main-root">
              {this.props.loggedIn ? (
                ...
                <Card className="teal lighten-5" title="Welcome to CVTasks! 👋">
                  To start jotting down tasks, log in using the button on the top right.
                </Card>
              )}
            </Col>
          </Row>
...
function LogoutButton(props) {
  function handleLogoutClicked(event) {
    api(
      ...
      undefined,
      (response) => {
        console.log(["Logging out complete", response]);
    ...
    <Modal
      actions={[
        ...
        <Button
          ...
          onClick={(e) => handleLogoutClicked(e)}
        >
          Log out
        </Button>,
      ]}
      header={`Confirm logout`}
      trigger={<Button node="a">Logout</Button>}
    >
      <p>Really log out?</p>
    </Modal>
  );
...
function SortChooser(props) {
  return (
    ...
    >
      {Array.from(Object.entries(sortNames), ([id, name]) => (
        <a key={id} value={id} onClick={props.onChange} href="#">
          {name}
        </a>
      ))}
    </Dropdown>
